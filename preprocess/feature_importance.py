...
def feature_select(X_t, y_t, X_p, y_p, feat_list, num_feat=1, ignored_feat=[], estimator=LogisticRegression(max_iter=10000)):
    #  dropping not influencing features from list of features
    if ignored_feat:
        for feat in ignored_feat:
            feat_list.drop(feat_list.loc[feat_list == feat].index, inplace=True)
            
    #  fitting estimator using all features up to specified number
...
def process(X, y, path):
    ...
    #  creating list of features with their importance based on logistic regression ratings
    ...
    #  dividing important and not important features using noise-filled feature
    ...
    #  selecting most influencing features from important features
    #  if new feature doesn't improve precision and f1 score beyond tolerance, feature is ignored, else feature is added
    ...
    del a, b, X_t, X_p, y_t, y_p
    logging.info("Done")
    pickle.dump(final_feat,  open(path, 'wb'))